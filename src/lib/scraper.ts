...
export async function scrapeNovelDetail(slug: string): Promise<NovelDetail | null> {
  ...
  // Genre — clean count suffix
  const genres: string[] = [];
  $(".genres-content a, .genres a, a[rel='tag']").each((_, el) => {
    const g = cleanGenre($(el).text().trim());
    if (g && g.length > 0) genres.push(g);
  });
  // Also try wp-manga-genre links
  if (genres.length === 0) {
    $(".manga-genres a, .genres a, .wp-manga-genre a").each((_, el) => {
      const g = cleanGenre($(el).text().trim());
      if (g && g.length > 0) genres.push(g);
    });
  }
  ...
  // Chapters — scrape dari chapter list
  const chapters: { title: string; slug: string; label?: string; date?: string }[] = [];
  $(".wp-manga-chapter a, .chapter-list a, .list-chapter a, .main .chapter-list .row a, .version-chap a, .listing-chapters_wrap li a, ul.main.version-chap li a, .wp-manga-chapter-list a").each((_, el) => {
    const $el = $(el);
    const chTitle = $el.text().trim();
    const chHref = $el.attr("href") || "";
    if (!chHref.includes("/novel/")) return;
    const chSlug = chHref.replace(BASE_URL + "/novel/", "").replace(/\/$/, "");
    if (!chTitle || !chSlug) return;

    // Label (HTL/MTL)
    const labelEl = $el.closest("li, .chapter-item").find(".chapter-type, .label, .vol");
    const label = labelEl.text().trim() || undefined;

    // Date
    const dateEl = $el.closest("li, .chapter-item").find(".chapter-date, .date, .release-date, .chapter-time");
