...
export async function scrapeNovelDetail(slug: string): Promise<NovelDetail | null> {
  ...
  $("ul.main li a, .wp-manga-chapter a, .chapter-list a, .list-chapter a, .listing-chapters_wrap li a, ul.version-chap li a, .wp-manga-chapter-list a, ul li.wp-manga-chapter a").each((_, el) => {
    const $el = $(el);
