...
export default function ChapterPage() {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    fetch(`/api/novels/chapter?slug=${encodeURIComponent(slug)}&chapter=chapter-${chNum}`)
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (data?.content) setContent(data.content);
        else setError('Gagal memuat chapter.');
        setLoading(false);
      })
      .catch(() => { setError('Gagal memuat data'); setLoading(false); });
  }, [slug, chNum]);

  const prevLink = chNum > 1 ? `/novel/${slug}/chapter-${chNum - 1}` : null;
  const nextLink = `/novel/${slug}/chapter-${chNum + 1}`;
  ...
  return (
    <div className="max-w-3xl mx-auto px-3 sm:px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-6 mt-4">
        {prevLink ? (
          <Link href={prevLink} className="px-4 py-2 bg-white dark:bg-gray-800 text-sm font-medium border border-gray-200 dark:border-gray-600 rounded-lg hover:border-indigo-300 transition">
            ← Prev
          </Link>
        ) : (
          <span className="px-4 py-2 text-sm text-gray-300 dark:text-gray-600">← Prev</span>
        )}
        <Link href={nextLink} className="px-4 py-2 bg-white dark:bg-gray-800 text-sm font-medium border border-gray-200 dark:border-gray-600 rounded-lg hover:border-indigo-300 transition">
          Next →
        </Link>
      </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-100 dark:border-gray-700 animate-pulse">
          <div className="space-y-3">
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="h-4 bg-gray-200 dark:bg-gray-700 rounded" style={{ width: `${Math.random() * 40 + 60}%` }} />
            ))}
          </div>
        </div>
      )}
